resource "aws_instance" "vprofile-bastion" {
  ...
  provisioner "file" {
    source      = templatefile(".terraform/templates/db-deploy.tmpl", { rds-endpoint = aws_db_instance.vprofile-rds.address, dbuser = var.dbuser, dbpass = var.dbpass })
  ...
  connection {
    user        = "proj"
    private_key = file("C:/terraform-1aws-vprofile/vprofilekey")
    host        = self.public_ip
  }
